...
{
    public Image demonicIcon;
    public Image boomIcon;
    public Image fallenIcon;
    public Image spookIcon;
    ...
    void Start()
    {
        ...
        iconOfPlayer.transform.localPosition = new Vector3(player.transform.position.x, player.transform.position.y - (height / 4));
    }
	...
	void placeEnemies(){
		...
		foreach (GameObject enemy in GameObject.FindGameObjectsWithTag("EnemyDemonic"))
		{
            ...
            Image demonic = (Image)Instantiate(demonicIcon, this.transform);
            demonic.transform.localPosition = new Vector3(enemy.transform.position.x, enemy.transform.position.y - (height / 4));
            Destroy(demonic);
        }
